...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        $imageUploadPath = $_SERVER['DOCUMENT_ROOT'] . '/A10/assets/Images/products/' . $imageName;

        // Kiểm tra nếu thư mục uploads không tồn tại thì tạo
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . '/A10/assets/Images/products/')) {
            mkdir($_SERVER['DOCUMENT_ROOT'] . '/A10/Images/products/', 0777, true); // Tạo thư mục với quyền ghi
        }
